...
* **mongo-token-kafka-redis &rarr** Herda as mesmas necessidades da branch **mongo-token**. Apenas foi adicionado o **REDIS** no `docker-compose.yml` para ser utilizado pelo projeto `filmes-legendary-video-library` com a branch `consumo-legendary-video-library-token-kafka-redis`.
